...
BASE_DIR = Path(r'../../')
# RUN_ID = '20251011_Lilab_Nikon_pic_split_data'
...
# SUM_THRESHOLD = 3000 # SUM_THRESHOLD should be 5 * INTENSITY_ABS_THRE

# 调整后的参数设置
TOPHAT_KERNEL_SIZE = 7  # 增大核大小，适应信号点尺寸
TOPHAT_BREAK = 100      # 降低背景阈值，因为信号整体较弱
LOCAL_MAX_ABS_THRE = 100  # 显著降低局部极大值阈值
INTENSITY_THRE = None  # 使用85%分位数作为强度阈值
CHECK_SNR = False       # 开启SNR检查，帮助区分真实信号与噪声
SUM_THRESHOLD = 1000    # 降低总强度阈值
...
def extract_signal(image_big, pad_x, cut_x, pad_y, cut_y, 
    ...
    coordinates = extract_coordinates(image, local_max_thre=LOCAL_MAX_ABS_THRE, intensity_thre=INTENSITY_THRE) #quantile=QUANTILE)
...
def main(max_memory = 64): # unit: GB
    ...
    def extract_intensity(channels=CHANNELS, **kwargs):
        ...
        def process_channel(channel, check_snr=CHECK_SNR):
            ...
            coordinate, snr, image_data = extract_signal(image, pad_x, cut_x, pad_y, cut_y, 
                                                         snr=4, tophat_mean=tophat_mean_dict[channel], check_snr=check_snr)
    ...
    intensity = intensity.rename(columns={'cy5': 'R', 'TxRed': 'Ye', 'FAM': 'B'})
    ...
    #                       ((intensity['G/A'] >= G_THRESHOLD) & (intensity['Scaled_G']>G_ABS_THRESHOLD))]
